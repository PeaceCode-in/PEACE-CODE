  ...
  "relative isolate inline-flex items-center justify-center gap-2 rounded-full cursor-pointer transition-transform duration-300 ease-out tracking-tight disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        outline: "",
        secondary: "",
        ghost: "bg-transparent hover:bg-accent hover:text-accent-foreground rounded-md [&_.btn-liquid-lens]:hidden",
        link: "text-primary underline-offset-4 hover:underline rounded-md [&_.btn-liquid-lens]:hidden",
      },
      size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, children, variant, size, asChild = false, contentClassName, glassColor, ...props }, ref) => {
    const filterId = React.useId().replace(/:/g, "")
    const Comp = asChild ? Slot : "button"
    const isGlass = variant !== "ghost" && variant !== "link"
        ...
        >
          {isGlass ? (
            <>
              ...
              <span className={cn(
                "relative z-10 w-full flex items-center justify-center gap-[inherit] select-none overflow-hidden text-ellipsis whitespace-nowrap",
                `btn-liquid-text-${filterId}`,
                contentClassName
              )}>
                {children}
...
// MetalButton is now an alias to Button for backward compatibility
const MetalButton = Button

export { Button, MetalButton, buttonVariants }
